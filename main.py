...
async def on_guild_join(guild):
    roles = ["Bot Pings", "Amateur Counter", "Intermediate Counter", "Advanced Counter", "Pro Counter", "Master Counter"]
    exisiting_roles = [role.name for role in guild.roles]
    for i in roles:
        if i not in exisiting_roles:
            await guild.create_role(name=i)
            print(f"Created a role named {i} in guild {guild.name}!")
...
async def leaderboard(ctx):
    ...
    for i in range(len(sorted_counter)):
        ...
        if i == 0:
            first = (f":first_place:: **{name}** - {score} points!\n")
            message += first
        elif i == 1:
            second = (f":second_place:: **{name}** - {score} points!\n")
            message += second
        elif i == 2:
            third = (f":third_place:: **{name}** - {score} points!")
            message += third
